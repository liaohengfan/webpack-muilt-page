...
export class OrbitControls extends EventDispatcher {
    object: Camera|PerspectiveCamera|OrthographicCamera;
    ...
    constructor (object: Camera, domElement?: HTMLElement, domWindow?: Window) {
        ...
        this.onMouseMove = ( event: ThreeEvent ) => {
            ...
            if ( this.state === STATE.ROTATE ) {
                ...
                const element = this.domElement === document ? (this.domElement as HTMLDocument).body : this.domElement;
        ...
        (this.domElement as HTMLElement).addEventListener( 'contextmenu', this.onContextMenu, false );

        (this.domElement as HTMLElement).addEventListener( 'mousedown', this.onMouseDown, false );
        (this.domElement as HTMLElement).addEventListener( 'wheel', this.onMouseWheel, false );

        (this.domElement as HTMLElement).addEventListener( 'touchstart', this.onTouchStart, false );
        (this.domElement as HTMLElement).addEventListener( 'touchend', this.onTouchEnd, false );
        (this.domElement as HTMLElement).addEventListener( 'touchmove', this.onTouchMove, false );
    ...
    tweenEnabled:boolean=false;
    tweenPhi:boolean=true;
    tweenTheta:boolean=true;
    sphericalEnd:Spherical=new Spherical();
    tweenUpdate() {
        ...
        if(this.tweenPhi){
            let valPhi:number=Math.abs(this.sphericalEnd.phi);
            if(valPhi<=0.0000002){
                this.sphericalEnd.phi=0;
                this.tweenPhi=false;
            }else {
                var phi = this.sphericalEnd.phi * .08;
        ...
        if(this.tweenTheta){
            let valTheta:number=Math.abs(this.sphericalEnd.theta);
            if(valTheta<=0.0000002){
                this.sphericalEnd.theta=0;
                this.tweenTheta=false;
            }else {
                var theta = this.sphericalEnd.theta * .08;
                this.sphericalEnd.theta -= theta;
                this.sphericalDelta.theta += theta;
            }
        }
        if(!this.tweenPhi&&!this.tweenTheta){
            this.tweenEnabled=false;
            console.log('tween update end');
            return;
        }
        this.update();
    }
    ...
    pan( deltaX: number, deltaY: number ) {
        ...
        if ( this.object instanceof PerspectiveCamera ) {
            // perspective
            const position = this.object.position;
            const cobject:PerspectiveCamera=(this.object as PerspectiveCamera);
            this.panInternalOffset.copy( position ).sub( this.target );
            var targetDistance = this.panInternalOffset.length();

            // half of the fov is center to top of screen
            targetDistance *= Math.tan( ( cobject.fov / 2 ) * Math.PI / 180.0 );
            ...
            // orthographic
            const cobject:OrthographicCamera=(this.object as OrthographicCamera);
            this.panLeft( deltaX * ( cobject.right - cobject.left ) / cobject.zoom / (element as any).clientWidth, this.object.matrix );
            this.panUp( deltaY * ( cobject.top - cobject.bottom ) / cobject.zoom / (element as any).clientHeight, this.object.matrix );
        } else {
    ...
    rotateLeft( angle: number ) {
        ...
        this.sphericalEnd.theta-=angle;
        //this.sphericalEnd.theta+=(angle*.5);
        this.tweenTheta=true;
        //(this.sphericalDelta as any).theta -= angle;
    }

    rotateUp( angle: number ) {
        this.tweenEnabled=true;
        this.sphericalEnd.phi-=angle;
        //this.sphericalEnd.phi+=(angle*.5);
